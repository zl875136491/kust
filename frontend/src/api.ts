...
async function mockRequest<T>(path: string, init?: RequestInit): Promise<T> {
  ...
  if (path.startsWith('/hosting/applications/') && path.endsWith('/redeploy') && init?.method === 'POST') { const id = path.split('/')[3]; const build = { ...mockHostingBuild, id: `mock-redeploy-${Date.now()}`, applicationId: id, status: 'running' as const, message: '正在使用现有镜像重新发布', createdAt: new Date().toISOString() }; mockApplications = mockApplications.map((application) => application.id === id ? { ...application, latestBuild: build } : application); return build as T; }
