...
function CreateApplicationModal({ open, clusters, credentials, capabilities, onClose, onCreated }: { open: boolean; clusters: Array<{ id: string; name: string }>; credentials: GitCredential[]; capabilities: HostingCapabilities; onClose: () => void; onCreated: (application: HostedApplication) => void }) {
  ...
  const hasExtendedBuildSettings = value.buildMode === 'static' || value.buildMode === 'custom';
  return <Modal open={open} onClose={onClose} title="新建托管应用" description="Kust 保存受限规格，Jenkins 构建不可变镜像，后端发布受控 Kubernetes 资源。" width="1120px" className="modal--hosting" dirty={Boolean(value.name || value.repositoryUrl)} closeDisabled={submitting} onSave={submit} onDiscard={() => setValue(initialApplication())} footer={(requestClose) => <><Button variant="ghost" onClick={requestClose}>取消</Button><Button variant="primary" icon={<Rocket size={16} />} onClick={() => void submit()} disabled={submitting}>{submitting ? '保存中' : '创建应用'}</Button></>}>
    <div className="hosting-form">
      <section><header><GitBranch size={17} /><div><strong>源码</strong><span>仓库凭证仅加密保存，构建时短暂提供给 Jenkins。</span></div></header><div className="form-grid"><label className="field"><span>应用名称</span><input value={value.name} onChange={(event) => set('name', event.target.value)} placeholder="orders-web" autoFocus /></label><label className="field"><span>分支或标签</span><input value={value.gitRef} onChange={(event) => set('gitRef', event.target.value)} placeholder="main" /></label></div><label className="field hosting-form__repository"><span>Git 仓库地址</span><input value={value.repositoryUrl} onChange={(event) => set('repositoryUrl', event.target.value)} placeholder="https://gitlab.example.com/team/orders-web.git" /></label><div className="form-grid"><div className="field"><span>Git 凭证</span><SelectMenu value={value.credentialId || ''} options={[{ value: '', label: '公开仓库，不使用凭证' }, ...credentials.map((credential) => ({ value: credential.id, label: `${credential.name} · ${credential.credentialType === 'ssh_key' ? 'SSH Key' : 'Access Token'}` }))]} onChange={(next) => set('credentialId', next || undefined)} aria-label="Git 凭证" /></div></div></section>
      <section><header><PackageCheck size={17} /><div><strong>构建</strong><span>{selectedMode.detail}</span></div></header><div className="build-mode-grid">{modeOptions.map((item) => <button key={item.value} type="button" className={item.value === value.buildMode ? 'is-active' : ''} onClick={() => set('buildMode', item.value)}><strong>{item.label}</strong><small>{item.detail}</small></button>)}</div><div className="hosting-build-editor"><label className="field hosting-build-editor__environment"><span>构建环境变量</span><textarea rows={10} value={environmentText(value.buildEnvironment)} onChange={(event) => set('buildEnvironment', parseEnvironment(event.target.value))} placeholder="ONNXRUNTIME_NODE_INSTALL_CUDA=skip" spellCheck={false} /></label><div className="hosting-build-editor__settings"><label className="field hosting-switch-field"><span>Root 兼容</span><span className="hosting-switch-control"><small>允许官方入口完成必要的初始化。</small><input type="checkbox" checked={value.runtimeProfile === 'root_compatible'} onChange={(event) => set('runtimeProfile', event.target.checked ? 'root_compatible' : 'non_root')} /><i /></span></label><label className="field"><span>源码子目录（可选）</span><input value={value.sourceSubdirectory || ''} onChange={(event) => set('sourceSubdirectory', event.target.value)} placeholder="apps/web" /></label>{hasExtendedBuildSettings && <><label className="field"><span>构建命令</span><input value={value.buildCommand || ''} onChange={(event) => set('buildCommand', event.target.value)} placeholder="npm ci && npm run build" /></label><label className="field"><span>产物目录</span><input value={value.outputDirectory || ''} onChange={(event) => set('outputDirectory', event.target.value)} placeholder="dist" /></label></>}</div></div></section>
      <section><header><Rocket size={17} /><div><strong>部署与访问</strong><span>平台仅生成 Deployment、Service 和 HTTPRoute。</span></div></header><div className="hosting-deployment-grid"><div className="field hosting-deployment-grid__cluster"><span>目标集群</span><SelectMenu value={value.clusterId} options={clusters.map((cluster) => ({ value: cluster.id, label: cluster.name }))} onChange={(next) => set('clusterId', next)} aria-label="目标集群" /></div>{capabilities.allowedNamespaces.length ? <div className="field hosting-deployment-grid__namespace"><span>命名空间</span><SelectMenu value={value.namespace} options={capabilities.allowedNamespaces.map((namespace) => ({ value: namespace, label: namespace }))} onChange={(next) => set('namespace', next)} aria-label="命名空间" /></div> : <label className="field hosting-deployment-grid__namespace"><span>命名空间</span><input value={value.namespace} onChange={(event) => set('namespace', event.target.value)} /></label>}<label className="field hosting-deployment-grid__port"><span>容器端口</span><input type="number" min={1} max={65535} value={value.containerPort} onChange={(event) => set('containerPort', Number(event.target.value))} /></label><label className="field hosting-deployment-grid__health"><span>健康检查路径</span><input value={value.healthPath} onChange={(event) => set('healthPath', event.target.value)} /></label><label className="field hosting-deployment-grid__route"><span>HTTPRoute Path</span><input value={value.routePath} onChange={(event) => set('routePath', event.target.value)} placeholder="/apps/orders-web" /></label><label className="field hosting-switch-field hosting-deployment-grid__auto-deploy"><span>启用自动部署</span><span className="hosting-switch-control"><small>分支推送时自动触发构建。</small><input type="checkbox" checked={value.autoDeploy} onChange={(event) => set('autoDeploy', event.target.checked)} /><i /></span></label></div></section>
    </div>
  </Modal>;
}
