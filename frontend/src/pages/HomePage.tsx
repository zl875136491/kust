import { Boxes, CirclePlus, FolderKanban, Pencil, RefreshCw, Server, Trash2 } from 'lucide-react';
import { useState } from 'react';
import { useNavigate } from 'react-router-dom';
import { useData } from '../data-context';
import type { Cluster } from '../types';
import { AddClusterModal, EditClusterModal } from '../components/ClusterModals';
...
export function HomePage() {
  ...
  const [editTarget, setEditTarget] = useState<Cluster>();
  ...
  return (
    <div className="page home-page">
      <header className="page-header">
        ...
        <div className="page-actions"><IconButton label="刷新集群" onClick={() => void refreshClusters()}><RefreshCw size={18} /></IconButton>{canManageClusters && <Button variant="primary" aria-label="添加集群" icon={<CirclePlus size={17} />} onClick={() => setAddOpen(true)}>添加集群</Button>}</div>
      </header>
      ...
      {loadingClusters ? <Spinner label="正在加载集群" /> : clusterError ? (
        ...
        <>
          ...
          ))}{canManageClusters && <button className="recent-cluster recent-cluster--add" onClick={() => setAddOpen(true)}><CirclePlus size={25} /><span>添加集群</span></button>}</div></section>}
          ...
          <section className="home-section cluster-list-section">
            ...
            {clusters.length === 0 ? <EmptyState icon={<Server size={24} />} title="还没有集群" action={canManageClusters ? <Button variant="primary" onClick={() => setAddOpen(true)}>添加集群</Button> : undefined} /> : (
              <div className="cluster-table-wrap"><table className="cluster-table"><thead><tr><th>名称</th><th>状态</th><th>警告</th><th>Kubernetes 版本</th><th>API Server</th><th aria-label="操作" /></tr></thead><tbody>{clusters.map((cluster) => (
                <tr key={cluster.id} onClick={() => navigate(`/cluster/${cluster.id}`)}>
                  ...
                  <td onClick={(event) => event.stopPropagation()}>{cluster.readOnly || cluster.preset ? <span className="preset-badge">预设</span> : canManageClusters ? <div className="cluster-row-actions"><IconButton label={`编辑 ${cluster.name}`} onClick={() => setEditTarget(cluster)}><Pencil size={16} /></IconButton><IconButton label={`移除 ${cluster.name}`} onClick={() => setDeleteTarget(cluster)}><Trash2 size={16} /></IconButton></div> : null}</td>
                </tr>
              ))}</tbody></table></div>
            )}
          </section>
      ...
      <AddClusterModal open={canManageClusters && addOpen} onClose={() => setAddOpen(false)} />
      <EditClusterModal cluster={canManageClusters ? editTarget : undefined} onClose={() => setEditTarget(undefined)} />
