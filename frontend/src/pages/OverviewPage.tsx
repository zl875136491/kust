...
export function OverviewPage() {
  ...
  const metricsQuery = useQuery({
    queryKey: ['metrics-summary', clusterId],
    queryFn: () => api.metricsSummary(clusterId),
    enabled: Boolean(clusterId),
    refetchInterval: 60_000,
  });
  ...
  return (
    <div className="page overview-page">
      ...
      <section className="overview-stats" aria-label="集群指标">
        <CircularStat label="CPU" value={metricsQuery.data?.available ? Math.min(metricsQuery.data.cpuMillicores / 10, 100) : undefined} display={metricsQuery.data?.available ? `${metricsQuery.data.cpuMillicores}m` : '--'} detail={metricsQuery.data?.available ? 'Metrics API 当前使用量' : 'Metrics API 未启用'} tone="#3578c4" icon={<Cpu size={18} />} />
        <CircularStat label="内存" value={metricsQuery.data?.available ? Math.min(metricsQuery.data.memoryBytes / (1024 * 1024 * 1024) * 10, 100) : undefined} display={metricsQuery.data?.available ? `${(metricsQuery.data.memoryBytes / (1024 * 1024 * 1024)).toFixed(1)} GiB` : '--'} detail={metricsQuery.data?.available ? 'Metrics API 当前使用量' : 'Metrics API 未启用'} tone="#8a63b8" icon={<MemoryStick size={18} />} />
